...
export function renderProgressResponseCard(
  ...
  // Strip the agent_progress_v1 metadata: the combined progress+answer shape must
  // render through the ordinary Adaptive Card path, not the specialized layout.
  const baseCard: Record<string, unknown> = { ...progress.card };
  delete baseCard.metadata;
