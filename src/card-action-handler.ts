...
import { CARD_INTERACTIVE_PROFILE } from './octo/types.js';
...
/** cc is single-bot per process; the account axis just guards a stale click after a token rotation. */
...
export async function handleCardAction(params: Params): Promise<CardActionHandleResult> {
  ...
  if (isReasoningControl) {
    ...
    // session to claim, no operator input to validate and no status frame to write back. cc has
    // not ported the reasoning progress cards (A6/A7), so this branch is a defensive no-op ACK for
    // cards already sitting in channels and for catalogs that still advertise the controls. ACK
    // means queue consumption only — never a lookup/dispatch on the interactive-card store below.
    params.log?.warn?.(
