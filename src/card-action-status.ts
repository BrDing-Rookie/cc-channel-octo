...
export function renderCardActionStatus(params: StatusParams): { card: Record<string, unknown>; plain: string } {
  ...
  // Terminal state: strip the Action.Submit set so a settled card can no longer be clicked.
  const cardWithoutActions: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(params.card)) {
    if (key !== "actions") cardWithoutActions[key] = value;
  }
