...
/** Minimal shapes of the render / action-status modules `runSink` drives (base or head). */
interface RenderModule {
  sanitizeErrorText: (input: string) => string;
  summarizeToolParams: (tool: string, params: Record<string, unknown>) => string;
}
interface ActionStatusModule {
  renderCardActionStatus: (opts: Record<string, unknown>) => { plain: string };
}

export function runSink(render: RenderModule, actionStatus: ActionStatusModule, sink: ParitySink, input: string): string {
