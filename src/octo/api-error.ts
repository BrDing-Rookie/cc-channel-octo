 ...
 *
 * Ported from openclaw-channel-octo `src/api-error.ts` to give cc's shared HTTP
 * helper a 429-aware error type. The status code and the server's rate-limit hints
 * used to survive only as text inside the message, so every caller that needed them
 * re-parsed the string. This carries them as fields while keeping the historical
 * message wording (`Octo API <path> failed (<status>): <body>`), which the existing
 * regex parser {@link httpStatusFromApiFetchError} and its callers still depend on.
 */
